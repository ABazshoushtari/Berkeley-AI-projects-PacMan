    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i + 1]]
        "*** YOUR CODE HERE ***"
        mostPossiblePosition = []
        for position in livingGhostPositionDistributions:
            mostPossiblePosition.append(max(position, key=position.get))

        best_action = None
        for position in mostPossiblePosition:
            for action in legal:
                successor = Actions.getSuccessor(pacmanPosition, action)
                if self.distancer.getDistance(successor, position) < self.distancer.getDistance(pacmanPosition, position):
                    best_action = action
        return best_action
